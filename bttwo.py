...
class bttwo():
    ...
    def all_url(self,url):
        ...
        max_page_url=BeautifulSoup(start_html.text,'lxml').find('a',class_='extend')['href']
        ...
        for page_num in range(1,int(max_page_num)+1):
            ...
            all_movie_a=BeautifulSoup(page_html.text,'lxml').find('div',class_='bt_img').find('ul').find_all('a')
            s = set()
            ...
            for movie_url in s:
                try:
                    ...
                    download_field = BeautifulSoup(movie_html.text,'lxml').find('div',class_='ypbt_down_list')
                    title = download_field.find('a')['title']
                    all_bt_a = download_field.find_all('a')
                except:
                    print(u'获取',movie_url,'出错，已跳过')
                    continue
                title.replace(':','：')
                print(u'开始保存',title)
                ...
                for bt_a in all_bt_a:
                    ...
                    size2=bigsezi[2:4]
                    try:
                        self.save(name,dataid,size1,size2)
                    except:
                        print(u'保存',name,u'出错,已跳过')
                        continue

    def save(self,name,dataid,size1,size2):
        print(u'开始保存',name)
